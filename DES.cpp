...
unsigned char* DES::encrypt(const unsigned char* plaintext)
{
	...
	unsigned char* paddedPlaintext = (unsigned char*)plaintext;

	//Declare an array DES_LONG block[2];
	DES_LONG block[2];

	//Use ctol() to convert the first 4 chars into long; store the result in block[0]
	block[0] = ctol((unsigned char*)plaintext);
	//Use ctol() to convert the second 4 chars into long; store the result in block[1]
...
unsigned char* DES::decrypt(const unsigned char* ciphertext)
{
	...
	//Perform des_encrypt1 in order to decrypt the block using this->key (see sample codes for details)
